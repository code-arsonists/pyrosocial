...
class Members extends Public_Controller {
    ...
    public function index()
    {
        ...
        $pagination = create_pagination('pyrosocial/members', $this->user_m->count_by($base_where));

        // Using this data, get the relevant results
        $this->db
                ->select('users.id, users.created_on, users.last_login,ps_friends.friend_id,
                        IF('.$this->profile_tbl.'.last_name = "", '.$this->profile_tbl.'.first_name, CONCAT('.$this->profile_tbl.'.first_name, " ", '.$this->profile_tbl.'.last_name)) as full_name,
                        IFNULL(IF('.$this->friends_tbl.'.friend_id = ' . $user_id . ', '.$this->friends_tbl.'.friend_id, '.$this->friends_tbl.'.user_id),0) AS is_friend, ps_friends.is_confirmed', FALSE)
                ->join('profiles', 'profiles.user_id = users.id', 'left');

        if ($this->input->post('mfilter') == 'friends')
        {
            // DONT SHOW OURSELVES
            $this->db
                    ->join('ps_friends', '('.$this->friends_tbl.'.user_id = users.id OR '.$this->friends_tbl.'.friend_id = users.id', 'left')
                    ->where('users.id !=', $this->current_user->id)
                    ->where('ps_ps_friends.is_confirmed', 1);
        }
        elseif ($this->input->post('mfilter') == 'requests')
        {
            $this->db
                    ->join($this->friends_tbl, $this->friends_tbl.'.friend_id = users.id')
                    ->where($this->friends_tbl.'.user_id', $this->current_user->id)
                    ->where('ps_friends.is_confirmed', 0);
        }
        else
        {
            $this->db->join('ps_friends', $this->friends_tbl.'.user_id = '.$this->db->dbprefix('users').'.id OR '.$this->friends_tbl.'.friend_id = '.$this->db->dbprefix('users').'.id', 'left');
        }
        ...
        } else $this->template->append_js("module::core_scripts.js");
    ...
    function request()
    {
        ...
        else
        {
            ...
            elseif (!$friend_status && $this->db->insert('friends', array('user_id' => $this->current_user->id, 'friend_id' => $friend_id, 'date_created' => now())))
            {
                $response = array(
                    'status' => 'ok',
    ...
    function confirm()
    {
        ...
        else
        {
            ...
            {
                if ($this->db->update('friends', array('is_confirmed' => 1)))
                {
